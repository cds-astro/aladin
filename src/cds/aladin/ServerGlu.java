...
public class ServerGlu extends Server implements Runnable {
   ...
   String system;       // appel syst�me dans le cas d'un enregistrement concernant une application locale, null sinon
   String dir;          // R�pertoire d'ex�cution du system, null sinon
   StringBuffer record;
   ...
   int baseUrlIndex;    // Indice du param�tre dont le type est BaseUrl(URL|MOCID)
...
//      try { this.aladinMenuNumber=Double.parseDouble(aladinMenuNumber); } catch( Exception e) { }
      ...
      // Dans le cas o� le Popup associ� est IVOA..., le serveur sera
      // consid�r� comme cach� (accessible uniquement en Discovery mode)
...
//							}
         ...
         // Construction d'un champ de texte libre
            ...
            if( paramDataType!=null && paramDataType[i]!=null ) {
               ...
               // Certain component ne doivent pas �tre affich� (tel le target)
...
//			co.setBounds(XTAB2,y,XWIDTH-XTAB2,HAUT); y+=HAUT+MARGE;
      ...
      // Gestion des filtres pr�d�finis s'il y a lieu
...
//				if (co.getName()!= null && !co.getName().equals("info")) {				}
  ...
  /** voir keyWordRequired */
   ...
   /** Retourne vrai si dans la liste des types de donn�es PK[] ont a au-moins une
    * fois un type donn� en param�tre
    * @param PK[] liste des types de donn�es
    * @param keyWord le type de donn�es recherch�
    */
...
//       return false;
    ...
    /** Retourne le premier indice dans la liste des types de donn�es PK[] qui a le type
     * de param�ter indiqu�
     * @param PK[] liste des types de donn�es
     * @param keyWord le type de donn�es recherch�
     */
   ...
   */
   ...
   /** Rep�rage des champs � afficher dans le formualire
...
//System.out.println("mode="+s);
         ...
         // input 1 plan => Choice � la place de TextField
...
//System.out.println("modeInput["+nbInput+"]="+modeInput[nbInput]);
        ...
        // Cas sp�cial pour rep�rer le champ qui devra recevoir le suffixe de l'URL HiPS
   ...
   /** Pour un serveur GLU on teste en premier si le nom du serveur pass�
    * en param�tre dans la commande script ne serait pas directement
    * l'identificateur de l'enregistrement GLU. Sinon on va regarder
    * en d�tail les chaines de descriptions
    */
   ...
   /** Retourne l'Item d'une JcomboBox qui contient la chaine "cr" (ou egalit� stricte
    * dans le cas d'un num�rique)
    * RA: "cr" est suppos� �tre en majuscules
    * Rq: traite le cas particulier "nn - chaine", o� le retour sera "nn" au lieu de "chaine"
    * Retourn null si non trouv� */
...
//         if( !flagNumeric && (s.indexOf(cr)>=0 || s1.indexOf(cr)>=0) || flagNumeric && s.equals(cr) ) {
   ...
   protected int createPlane(String target,String radius,String criteria,
      ...
      try {
         ...
         // Pr�-remplissage du champ concernant la date
         if( date!=null && date.getText().trim().length()==0 ) resolveDate(getDefaultDate());

         // D�coupage des crit�res dans un tableau
         Tok st = new Tok(criteria.trim(),",");
         String crit[] = new String[st.countTokens()];
         for( i=0; st.hasMoreTokens(); i++ ) {
            crit[i] = st.nextToken();
            
            //System.out.println("Crit�re "+(i+1)+" ["+crit[i]+"]");
         }

         Vector v = new Vector(10);    // Liste des crit�res finaux
         ...
         // Placement des crit�res label�s (ex: Survey=DSS1)
         for( i=0; i<crit.length; i++ ) {
            ...
            if( posEgal>0 && cr.charAt(posEgal-1)!='\\' ) {
               ...
               // Recherche du param�tre correspondant
               e = vc.elements();
         ...
         // Placement des crit�res non label�s, mais dont la valeur va �tre
         // trouv�e dans la liste de la JComboBox
         e = vc.elements();
         for( j=0; e.hasMoreElements(); j++ ) {
            JComponent c = (JComponent)e.nextElement();
         ...
         // Passage en revue des crit�res input non encore utilis�s et je les place
         // postionnellement dans les crit�res output en sautant les cases d�j� renseign�s
         e = vc.elements();
         JComponent c=(JComponent)e.nextElement();
         for( j=i=0; i<crit.length; i++ ) {
            if( crit[i]==null ) continue;		// d�j� utilis�
            ...
            // Si le component est tagu� en INPUT, on va remplacer le nom du plan saisie
            // par son URL (si possible)
            if( isFieldInput(c) ) {
               setSelectedItem(c,crit[i]);		// Je positionne le Choice � la main sinon
         ...
         // Passage en revue des crit�res manquants en output et positionnement de la valeur
         // par d�faut correspondante si elle peut �tre d�termin�e, sinon ""
         e = vc.elements();
         for( j=0; e.hasMoreElements(); j++ ) {
            c = (JComponent)e.nextElement();
            if( v.elementAt(j)!=null ) continue; // d�j� renseign�
         ...
         // R�solution d'un param�tre de type BaseUrl(URL|MOCID)
         // Si effectivement il s'agit d'un ID et que celui-ci retourne une ou plusieurs URLs s�par�es par des TAB,
         // on bouclera sur chaque URL en la rempla�ant successivement dans le vecteur des param�tres qui va bien.
         // En revanche, s'il n'y a qu'une URL retourn�e, ou directement pass�e en param�tre, la boucle se limitera
         // � un seul tour.
         Tok tokUrlList=null;
         if( baseUrlIndex>=0 ) {
            ...
            if( !s.startsWith("http://") && !s.startsWith("https://") ) {
               ...
               if( urlList!=null ) {
                  resourceId = s;   // m�morisation au cas de redo (cf. PlanCatalog.redo..)
         ...
         boolean encore=true;  // D�termine la sortie de la boucle
         int n=-1;             // Num�ro du dernier plan cr�� dans la pile, sinon -1
...
//               }
               ...
               // IL est possible que l'url soit en fait une collection d'urls s�par�es par un TAB
               // notamment lorsque la r�solution de l'URL a �t� faite via MocServer
               try {
            ...
            // On modifie dynamiquement l'URL � utiliser en v�rifiant que ce n'est pas la fin de la liste
   ...
   private void submit1(boolean flagDoIt, boolean userReady) {
      ...
      Enumeration e;
      String code=null;
      boolean flagScriptEquiv=true;	// Par d�faut, il existe tjs une commande script �quivalent
         ...
         try {
            ...
            if (this.boundaryAreaStcs != null && !this.boundaryAreaStcs.isEmpty()) { //current config to check target limits
            	if (this.posBounds == null) {
					...
					this.posBounds = aladin.createMocRegion(stcObjects,-1, true);
				}
      ...
      while( e.hasMoreElements() ) {
         ...
         if( isFieldHiPS(c) ) {
            ...
            // Si la valeur est pr�c�d�e d'un "XXX - valeur", c'est XXX qui sera utilis�
            // en tant que valeur.
         ...
         // Mise � jour des crit�res de la commande script �quivalente
...
//    	  e = v.elements();
      ...
      // Il s'agit d'un appel syst�me ?
      if( system!=null ) { exec(label, p==null?"" : p.toString() ); return; }
      
      // G�n�ration de la commande script �quivalent
          ...
          try{
            ...
            // Traitement des images par lot
             if( tree!=null && !tree.isEmpty() ) {
			   ...
			   // Chargement des descriptions des images disponibles dans thread s�par�
			   } else {
				   waitCursor();
				   uT = u;
...
//            lastPlan = aladin.calque.createPlan(u + "", label, "provided by " + institute, this);
   ...
   /** Appel effectif pour la cr�ation du plan.
    * Cette m�thode est surcharg� dans le cas d'un service utilisant un POST multipart => ex: ServerSextractor */
...
// 		  }
   ...
   /** Lance l'�x�cution de l'application system */
   ...
   protected Plan lastPlan=null; 	// Dernier plan cr��

	URL uT; // url � passer au thread
	String posT; // position � passer au thread
...
//			MyInputStream in = new MyInputStream(u.openStream());
   ...
   /** Interrogation en mode discovery */
	protected MyInputStream getMetaData(String target,String radius,StringBuffer infoUrl) throws Exception {
      ...
      // Positionnement des n�cessaires au target et radius pour le formulaire
  ...
  /** Nettoyage du formulaire */
   protected void clear() {
      ...
      for( int i=0; i<c.length; i++ ) {
         ...
         // On prend la premi�re valeur au cas o�
   ...
   protected boolean updateWidgets() {
      if( flagTAP ) {
//         System.out.println("Je dois r��diter la chaine ADQL");
         submit1(false, false);
      }
