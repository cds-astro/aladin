...
public class ServerFile extends Server implements XMLConsumer {
   ...
   protected ServerFile(Aladin aladin) {
      ...
      // Indication du component � maximiser
      setMaxComp(sc);
   }
   ...
   /** Retourne le nom du fichier ou de l'url en enlevant un �ventuel suffixe
    * [nnn] indiquant une extension particuli�re d'un MEF */
   ...
   private String getLabelFromFile(String f) {
      ...
      // Suppression d'une extension �ventuelle
      i = label.lastIndexOf('.');
   ...
   /** Creation d'un plan issu d'un chargement d'un fichier AJ, fits ou autre
    * @param f path du fichier
    * @param resNode noeud d�crivant le fichier � charger, peut �tre <i>null</i>
    */
	...
	protected int creatLocalPlane(String f, String label, String origin, Obj o, ResourceNode resNode, InputStream is,
      ...
      try {
         ...
         try {
            ...
            // Analyse du contenu d'un r�pertoire local
            ...
            //Pas de stream, juste une URL en HTTP => d�couverte du contenu et/ou cr�ation du stream
            if( is==null && (f.startsWith("http:")||f.startsWith("https:")) ) {

               // S'agirait-il d'un HiPS distant ?
               try {
                  gSky = new TreeObjDir(aladin, f, true);
//                  System.err.println("!!! gSky="+gSky.toString1());
               ...
               if( gSky==null ) {
                  ...
                  try { in = Util.openStream(u); } catch( Exception e ) {
                     ...
                     if( s.startsWith(Util.HTTPERROR) ) {
                        ...
                        if( code==400 ) throw new Exception("Bad request ("+code+" HTTP error)"); 
                        if( code>=401 && code<=403 ) throw new Exception("Data access forbidden ("+code+" HTTP error)"); 
                        if( code==404 ) throw new Exception("Resource not found ("+code+" HTTP error)"); 
                        if( code>=405 ||code>=500 ) throw new Exception("Server error ("+code+" HTTP error)"); 
                     }
            ...
            // support FTP --> �a fonctionne avec par exemple une URL du type ftp://user:passwd@server/....
            ...
            if( gSky==null ) {
               ...
               // Dans le cas d'un chargement d'une r�gion ou d'un ancien contour, on va forcer la cr�ation d'un nouveau plan
               ...
               if( (type & (MyInputStream.AJS|MyInputStream.AJSx|MyInputStream.UNKNOWN))!=0) aladin.command.readFromStream(in);
                  ...
                  // un nouveau plan sera cr�� sur la pile si la description contient les PARAM de position
                  boolean newPlane = (n=aladin.processFovVOTable(in,null,true))>=0;
                  // si on a juste ajout� un FOV � la liste des FOV charg�s,
                  // on se place sur cet onglet et on s�lectionne le FOV en question
                  if( !newPlane ) {
                     n=-2;  // Pour �viter une erreur via VOAPP (--> c'est du propre !)
                  ...
                  // C'est peut �tre une image native ?
                  ...
                  // C'est peut �tre un fichier de properties ?
               } else if( (type & MyInputStream.PROP)!=0 ) {

                  // Dans le cas d'un fichier properties, on passe le path ou l'url pour 
                  // fournir l'acc�s au HiPS associ� dans le cas o� il n'est pas d�crit
                  ...
                  // C'est peut �tre un planBG via HTTP
   ...
   private int footprintIdx=-1; // Indice de colonne d'un �ventuel footprint associ� � chaque source
   private ViewMemoItem vmi;  // ViewMemoItem courant
   private int firstView;   //indice de la premi�re vue � afficher
   ...
   private boolean flagCatalogSource=false; // true si le plan courant est TOOL et que celui-ci contient �galement un catalogue de sources
      ...
      switch(typePlan) {
         ...
         case Plan.IMAGE:
            ...
            if( plan instanceof PlanBG ) {
               ...
               if( (s=(String)atts.get("hipsbody"))!=null )      pbg.body = s;  // Pour compatibilit� - � virer quand on veut
...
//               System.out.println("footprint in "+footprintIdx+" on "+f);
   ...
   public void endElement(String name){
      if( name.equals("PLANE") ) {
         ...
         if( plan!=null ) {
            ...
            // pour prendre en compte pas la suite les filtres d�di�s
         ...
         // mise � jour des diff�rents filtres m�moris�s
         if( pFilter!= null ) {
            ...
            // un peu bourrin, mais c'�tait le plus facile
         ...
         // mise � jour des diff�rents filtres d�di�s m�moris�s
         if( pdf!=null ) {
   ...
   private int getSource(char [] ch, int cur, int end) {
      ...
      // modif Thomas (le trim() supprimait la valeur de certains champs "blancs" situ�s en bout de ligne)
      ...
      // Par d�faut on active l'affichage du label des tags (du grand bricolage PF 5/9/2022)
      if( typePlan==CATALOGTOOL && glu!=null && glu.endsWith("|Tags>")) o.setWithLabel(true);
      
      // G�n�ration d'un footprint associ� � la source
      if( footprintIdx!=-1 ) {
   ...
   private int getTool1(char [] ch, int cur, int end, boolean ajtool) {
      ...
      // Ajout de l'objet dans le plan courant
      Position o=null;
      if( typeTool.equals("tag") )        o = ( new Repere(plan) );   // Pour compatibilit� avec les versions <7
      else if( typeTool.equals("text") )      o = ( new Tag(plan) );      // Pour compatibilit� avec les versions <7
   ...
   /** Lecture du MOC ASCII, et l'associe au plan courant. C'est procedure peut �tre appelee plusieurs
    * fois en sequence.
    * @param ch
    * @param cur
    * @param length
   ...
   // m�moire des PlanFilter � mettre � jour et des d�finitions correspondantes (cl� : PlanFilter, value : d�finition)
   Hashtable pFilter;
   /** Mise en place du script sur le plan filtre en cours de construction */
   private void setScript(String script) {
      // d�codage des entit�s sp�ciales
      ...
      // m�morisation du PlanFilter et de la d�f. associ�e
      pFilter.put(plan, script);

   }

   // Type de plan modifi� pour prendre en compte les catalogues inclues dans les tools
   ...
   public void characters(char ch[], int start, int length){
      ...
      if( inValue ) {
         ...
         // thomas, 22/11/06 : les sauts de ligne ont �t� remplac�s par un "\n" dans le .aj
         //Il faut donc les d�coder
