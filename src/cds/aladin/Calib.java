...
public final class Calib  implements Cloneable {
   ...
   public Calib (HeaderFits hf) throws Exception {
      ...
      if( proj==-1 && hf.getStringFromHeader("GRIDTYPE")!=null ) {
         proj=CAR;
         Aladin.aladin.command.printConsole("!!! No projection specified in CTYPE, GRIDTYPE present => assuming "+projType[proj]);
      }

      if( proj==-1 && hf.getStringFromHeader("CRVAL1")!=null ) {
         proj=TAN;
         Aladin.aladin.command.printConsole("!!! No CTYPE keyword: assuming "+projType[proj]);
      }
