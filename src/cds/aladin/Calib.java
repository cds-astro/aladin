...
public final class Calib  implements Cloneable {
   ...
   //     * AJOUT PF nov 09 pour supporter les projections lin�aires CRVAL+CDELT, mï¿½me
   ...
   // PF. 12/06 - Modif li�es � l'utilisation de la nouvelle classe de Fox
   // pour les conversions de coordonn�es. On cr�e � l'avance les diff�rents
   ...
   public Calib (HeaderFits hf) throws Exception {
    ...
    //  System.out.println("aladin "+aladin);
      ...
      try {
         ...
         // Probl�me dans le cas ou le type est UNknown ? 
 ...
 *FastMath.cos((X1-alphai)*Math.PI/180.))
 / deno; 
  ...
  // System.out.println("center "+centerRadec[0]+" "+centerRadec[1]); 
   ...
   // PF - Jan 2011 - La m�thode de calibration DSS ne marche pas actuellement avec les imagettes
   // => dans les mains de Fran�ois B. En attendant, je fais un gros patch
...
//               break;
 ...
 //                    for (int j = 0 ; j < order-powx + 1 ; j++)
  ...
  //             for (int order = 2;  order < order_bp+1 ; order++)
   ...
   // Pierre - sept 2011 : depuis la derni�re mouture de Calib, il peut y avoir des valeurs n�gatives
   //                    inc[0]= incA ;
   //                    inc[1]= incD ;
   inc[0]= Math.abs(incA) ;
   inc[1]= Math.abs(incD) ;
 ...
 //  System.out.println(:qpoly[0]+" "+ xydpoly[0]);
   ...
   // Le mot cl� RADECSYS n'est concern� que par les syst�m� �quatoriaux
   // Modif PF Jan 2011
 ...
 */
   ...
   if(( aladin == 0)||(aladin == 3)) {
      ...
      // du bas et il y a la hauteur � enlever en prime ...
      Ycen -= (ynpix -offy -h ) ;
   }
   else {
      Xorg += offx *incX ;
   ...
   // thomas, 19/11/2007
   // TODO : Fran�ois, peux tu me v�rifier cette m�thode ?
   ...
   /** Code modifi� par Pierre F. Juillet 2010 - � v�rifier par Fran�ois B. SVP - code original ci-dessous
 * Remodifi� par Pierre F. en Mars 2011 - toujours � v�rifier par Fran�ois B. SVP */
