...
//                        +v+"\" => d�j� fait !");
   ...
   long timeStartTable;                 // Date du d�but du parsing de la table
   ...
   protected void postJob(double rajc,double dejc,double rm,boolean setSourceType) {
      ...
      if( flagXY ) {
         ...
         plan.hasXYorig=true;
         flagXY=false;   // on resete ce flag sinon �a posera souci en cas de positionnement ult�rieur d'une projection via RA,DEC manuel
      ...
      // Positionnement du centre que si ce n'est pas la valeur par d�faut 0,0
   ...
   private int indexDataProductType=-1; // Position de la colonne pour un dataproduct_type �ventuel
   private int indexServiceDef=-1 ;
   private int indexSTC=-1;          // Position de la colonne pour un FOV STC �ventuel
...
//   }
   ...
   public void setRecord(double ra, double dec, double jdTime, String[] value) {
      ...
      try {
         ...
         if( flagFirstRecord ) {
            leg=null;   // Reinitialisation si plusieurs tables dans une m�me resource
...
//                       displayField.name = f.name;
            ...
            // En cas de plan HiPS catalog, la l�gende est globale et externe
            ...
            if( href==null && (value[i].startsWith("http://") || value[i].startsWith("https://") || value[i].startsWith("ftp://"))) {
               ...
               if( flagArchive==null ) {
                  ...
                  if( ucd!=null && Util.indexOfIgnoreCase(ucd,"Image_AccessReference")>=0 ) {
                     ...
                     indexAccessUrl=-2;  // Pour �viter de le traiter par la suite
                  }
               }
               ...
               // datalink uniquement bas� sur le nom de la colonne (genre Markus)
...
//            String text = (refText != null) ? refText : value[i];
            ...
            // Les TABs ne peuvent �tre pr�sents dans les valeurs individuelles (au risque de ne plus pouvoir relire les donn�es
        ...
        //	aladin.mesure.getFormInfo(5);
         ...
         if (idxSTCS>=0) {
            try {
               ...
               // Attention des petits rigolos (Markus) utilisent parfois des tableaux de r�els o� ils alternent lon/lat
...
//                  source.setValue(idxSTCS, val);
         ...
         // Peut �tre un FoV peut �tre tout de m�me g�n�r� par SIA1 ou SSA ?
         } else 
         ...
         // en fonction du type de donn�e de la colonne dataproduct_type et sinon du MIME type de la colonne "access_format"
...
// Code Chaitra => bien trop lent [le split est � fuir]
...
//         if( o instanceof Source && ((Source)o).isTagged() ) return true;
   ...
   /** Positionnement de l'�poque originale */
   ...
   /** Positionnement � l'avance du target finale => n�cessairement en ICRS */
...
//      flagTarget=false;
      ...
      if( ok && plan instanceof PlanBGCat ) {
         if( nb_o==0 ) setRecord(0, 0, Double.NaN, null);  // pour initialiser tout de m�me la l�gende
         return nb_o;
      }
   ...
   protected void createDefaultProj() {
      if( nb_o==0 ) { 
         ...
         try {
            // Si toute tombe dans la m�me cellule MOC de 1�, on fera une projection limit�e � 1�
            // sinon on fera large (on ne s'emb�te plus car g�n�ralement il y a un HiPS en dessous)
...
//         flagSIAV2 = (type & MyInputStream.SIAV2)!= 0;
      ...
      if ( flagDataLink ) {
    	...
    	//	 System.out.println("nb ressources "+parser.getResourceCount());
    	 ...
    	 // else System.out.println("succès") ;
      }
...
//	         if( ! (o[i] instanceof Source) ) continue;
   ...
   /** Insertion de la source apr�s la derni�re source de m�me l�gende, sinon � la fin */
   protected void insertSource(Source src) {
      for( int i=nb_o-1; i>=0; i-- ) {
         ...
         if( ((Source)o[i]).getLeg()==src.getLeg() ) {
            int n = nextIndex();
            for( int j=n; j>i+1; j--) o[j] = o[j-1];  // d�calage
            o[i+1] = src;
            return;
         }
      }
...
//         if( ! (o[i] instanceof Source) ) continue;
   ...
   // Recup�ration d'un it�rator sur les objets visibles dans la vue
