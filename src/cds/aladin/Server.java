 ...
 * @version 1.2 : 19 sept 2005 - gestion homog�ne du radius
...
public class Server extends JPanel
   ...
   // Les diff�rents status de la derni�re interrogation ALLVO
   ...
   static final int YOUTREACH = 60; // Ordonn�e du premier label en mode OUTREACH
   static final int MAXSELECTEDPLANE = 10; // Nombre max d'images � charger avant affichage un warning
   ...
   static final int XTAB1=10;		// Abscisse des labels des champs � saisir
   ...
   String ordre = "X";             // num�ro d'ordre du formulaire
   ...
   protected boolean DISCOVERY = false;  // true si le serveur doit �tre pris en compte en mode discovery
   ...
   // Reperage des TextFields indiquants le centre,la taille la date du champ et les �ventuels
   // champs input (utilis�s pour passer l'url associ� � un plan d�j� charg�)
   ...
   protected int modeInput[]; // M�morise le modeInput de chaque champ input
   protected int modeBand=0;
   protected int modePos = 0; 

   static final int NOMODE = 0;  // Aucun mode d�fini (pour target ou radius)
   ...
   static final int RADSQRd=32; // Field en carre (degr�s), 1 champ
   static final int RADBOXd=64; // Field en box (degr�s), 2 champs
   ...
   static final int YEARd = 4;		// Date en ann�es d�cimale
   ...
   protected double aladinMenuNumber=0;      // Num�ro d'ordre du serveur (ordre d'apparition dans le serveur selector)
   protected String title;		  // Le titre de la fen�tre
   protected String gluTag;          // Le Tag GLU (ActionName) associ� au serveur (ou null si aucun)
   protected String resourceId;    // L'identificateur de la ressource concern�e dans le cas d'une requ�te g�n�rique genre get TAP(ID,...)
   protected String aladinLabel="";       // le nom du serveur
   protected String description;         // les infos (une ligne) decrivant le serveur
   protected String verboseDescr;         // Description d�taill�e du serveur
   protected String institute;         // L'origine
   protected String aladinLogo;         // Le logo attach� au serveur (null si aucun)
   ...
   protected String filters[];    // Liste des filtres pr�d�finis selon la syntaxe suivate:
                                  //    # Description une ligne\nfilter Nom.Filter { contrainte { action }.... }
   JComboBox filtersChoice=null;     // La boite de choix du filtre pr�d�fini
   private Component maxComp=null;// Le component du Server qui doit �tre maximis�
   ...
   protected String statusUrl=null;    // Derni�re URL d'appel pour le mode ALLVO
   protected String statusError=null;  // Derni�re erreur
   protected Ball ball;                // Voyant d'�tat
...
//      aladinMenuNumber = ORDRE++;      // On se sert du createChaine pour initialiser le num�ro d'ordre d'apparition par d�faut
...
//      GRABIT         =aladin.chaine.getString("GRABIT");
   ...
   /** Positionnement du status du serveur de la derni�re requ�te AllVO */
   ...
   /** Interpr�te la coordonn�es et ex�cute S�same si n�cessaire */
   ...
   * @param le target pass� en param�tre
...
//            System.out.println("c.getSexa="+c.getSexa()+" coo[0]= "+c.getRA(' ')+" coo[1]="+c.getDE(' '));
    ...
    * @param updateRadius true s'il faut aussi mettre � jour le champ radius
    */
   protected void resolveRadius(String s,boolean updateRadius) {
      ...
      // Mise � jour du champ radius g�n�rique
...
//    else if( (modeRad&(RADIUS|RADIUSd|RADIUSs))!=0 ) radius.setText( Coord.getUnit(rm/60.) );
      if( updateRadius && modeRad!=NOMODE ) {
      	try {
      		...
      		// r�solution du bug #213 (cf. http://pclx5:8080/support/issue213 )
      		radius.setCaretPosition(Math.min(radius.getCaretPosition(), newRadius.length()));
      	}
      	// r�solution de la demande #222 (cf. http://pclx5:8080/support/issue222 )
      	catch(Exception e) {}
    ...
    * (sert pour initialiser la valeur par d�faut pour les commandes scripts)
...
//      return s+".......................................";
   ...
   /** Construction du panel g�n�rique pour le Target/radius
    * Retourne la hauteur du panel
    * @param p le Panel � cr�er (de fait mis � jour)
   ...
   protected int makeTargetPanel(JPanel p,int mode) {
      ...
      //  Construction des �l�ments pour le Target
      if( !noTarget ) {
         l=XTAB2-10;
         if( forVizieR ) l=100;
      ...
      // Construction des �l�ments pour le Radius
      x=0;
      l = 55;
      String rad=RAD;
   ...
   /**
    * Fixe le focus sur l'un des �l�ments du formulaire
    * (on �conomise ainsi un click de souris)
    */
   ...
   // TODO : temporaire
   // sera supprim� apr�s refonte MetadataTree/BasicTree
   boolean initDone;
...
//           tree.init();
   ...
   /**
    * M�morise le component qui sera toujours maximis�
    * @param c Component � maximiser dans le panel
    */
   ...
   // true si on ne souhaite maximiser qu'en Y, sans toucher � la largeur
   private boolean maximizeInYOnly = false;


   /**
    * Maximise la taille du component maxComp m�moris� par setMaxComp en fonction
    * de la taille courante du Panel. D�cale les components situ�s en dessous
    */
...
public void layout() {
   ...
   /** Transformation des infos target/radius exprim�es dans les champs g�n�riques
  ...
  * @param confirm true - teste que le champ est bien renseign�
  */
   ...
   protected String getTarget(boolean confirm) {
      ...
      // Resolution d'un �ventuel Target Name ? (d�pendant du type de serveur)
   ...
   // Par d�faut, par de r�solution de nom
   // => C'est g�n�ralement le service distant qui assure la r�solution
   protected String resolveTargetName(String s) { return null; }
   
   // Cf les classes d�riv�es (=> resolveTargetName() qui appelle resolveTargetNameNow()
   protected String resolveTargetNameNow(String s) { 
      ...
      try {
         ...
         // On fait un changement de rep�re pour changer la position de la vue courante sur l'objet
    ...
    * @param confirm true - teste que le champ est bien renseign� et non <0
    */
...
//         }
   ...
   /**
    * Retourne le rayon exprim� par r en arcmin. r peut �tre suivi
    * d'une unit�. S'il n'y en a pas, l'unit� consid�r� sera celle
    * pass� en param�tre (ou celle du formulaire si non indiqu�)
    * @param r la chaine exprimant le rayon
    * @param modeRad l'unit� par d�faut
...
// Determination de l'unite
      ...
      // si l'utilisateur a pass� deux quotes simples, plut�t qu'une double
      ...
      // Si absence d'unit�, il s'agit de l'unit� par d�faut (modeRad)
      else {
...
//   }
   ...
   /** Retourne la rayon (en arcmin) associ� � un champ d�crit par la chaine s */
   static protected double getRM(String s) { return getRWM(s,0); }

   /** Retourne la largeur (en arcmin) associ� � un champ d�crit par la chaine s */
   static protected double getWM(String s) { return getRWM(s,1); }

   /** Retourne la hauteur (en arcmin) associ� � un champ d�crit par la chaine s */
   static protected double getHM(String s) { return getRWM(s,2); }

   /** Retourne la taille (en arcmin) associ� � un champ d�crit par une chaine
    * Si le champ est donn� sous forme d'un rayon on retournera le cas �ch�ant le carr� ENGLOBE,
    * et si le champ est donn� sous forme d'un rectangle on retournera le cas �ch�ant le cercle ENGLOBANT.
   ...
   static private double getRWM(String s,int mode) {
      ...
      // Le Champ est exprim� en Rayon (un seul param�tre)
      if( i<0 ) {
         rm = getAngleInArcmin(s,RADIUS); // Ca colle direct pour rm
         hm=wm = 2*rm;            // On prend la boite � l'ext�rieur du cercle pour hm,wm

      // Le Champ est exprim� en Rectangle (2 param�tres)
      } else {
         wm = getAngleInArcmin(s.substring(0,i),RADIUS);  // On r�cup�re la largeur
         hm = getAngleInArcmin(s.substring(i+1),RADIUS);  // On r�cup�re la hauteur
  ...
  /** Clear du formulaire (et reaffichage) */
   ...
   // Dans le cas de formulaire en deux �tapes, met en exergue
   // l'indication de l'�tape courante en fonction de l'arbre
   // des m�tadonn�es
   ...
   */
    ...
    */
	...
	/** Pr�-remplissage du champ baseUrl */
	protected void setBaseUrl(String s) {
	   if( baseUrl==null ) return;
	   baseUrl.setText(s);
	}

   /** Pre-remplissage du champ Date. Si c'est une valeur double, on consid�re
    * que c'est une ann�e d�cimale et on la convertit en JD, sinon on laisse
    * tel que.
    ...
    */
    protected void setDate(String s) {
       ...
       // On suppose que s est en ann�e d�cimale (via getEpoch() )
       try {
// Methode Fox
...
//          date.setText( Astrodate.YdToJD(Yd)+"" );
       ...
       // r�solution du bug #241 (cf. http://pclx5:8080/support/issue241 )
       try {
...
//				}
    ...
    /** Positionnement d'une valeur particuli�re (s) sur un component
     * de type INPUT. N�cessaire pour l'utilisation via Aladin script
     */
    ...
    /** Retourne la liste des plans concern� par un s�lecteur de type INPUT */
    protected Plan [] getInputPlane(JComponent c) {
       ...
       // Construction du vecteur r�sultat en fonction du type de input (IMG|IMGs|CAT|CATs)
    ...
    /** Retourne une chaine constitu�e de la concat�nation de tous les noms de plans
     * correspondant au "masque de nom de plan" indiqu� par le Component pass� en param�tre
     * Le s�parateur est l blanc
     */
    ...
    /** Retourne une chaine constitu�e de la concat�nation de toutes les URL de plans
     * correspondant au "masque de nom de plan" indiqu� par le Component pass� en param�tre
     * Le s�parateur est le | (pipe). Les �ventuels | se trouvant dans les urls seront
     * backslash�s
     * */
    ...
    /** Retourne une URL temporaire pour r�cup�rer des donn�es locales (via le tiny httpd)
     * Cette m�thode est surcharg�e pour les services qui supportent le POST multipart => voir par exemple ServerSextractor */
    protected String getUrl4LocalData(Plan p) {
       return Export.export(p);  // Serveur local
    }

     /** Backslash les �ventuels '|' */
     ...
     /** Retourne true si le Component pass� en param�tre concerne un des champs
	...
	protected boolean disableDateField(String toolTipText) {
     ...
     /** Retourne true si le Component pass� en param�tre concerne un
      * dhamp de date  */
     ...
     /** Retourne true si le Component pass� en param�tre ne sert qu'� rep�rer un champ qui
     ...
     /** Retourne true si le Component pass� en param�tre est input */
     protected boolean isFieldInput(JComponent c) {
        return getFieldInput(c)>=0;
     }

     /** Retourne l'indice dans input[] du component pass� en param�tre */
     ...
     /** Mise � jour dynamique des choices associ�s aux champs input en fonction
      ...
      * @param Choice le Choice � mettre � jour
      * @param v le vector contenant les plans dont les labels vont �tre utilis�s.
...
//      return true;
    ...
    * je d�quote les param�tres le cas �ch�ant. De toutes fa�ons, ils ne supportent
    * que des crit�res sans espaces
    * @param s la liste des crit�res (ex: ["DSS1",Fits])
    * @return la liste des crit�res d�quot�s (ex: [DSS1 Fits])
    */
   ...
   /** Retourne true si l'utilisateur a indiqu� qu'il y avait trop d'images s�lectionn�es
    * dans l'arbre */
...
//            && ((JButton)s).getActionCommand().equals(FrameServer.INFO)) showStatusReport();
  ...
  /** Retourne le Nom du server �ventuellement pr�c�d� par son Popup.
    * les / seront pr�fix�s par \, les \n seront remplac�s par un blanc,
    * les ... seront supprim�s en fin de popup.
   ...
   protected String [] getNomPaths() {
      ...
      // On met le Popup en pr�fixe s'il y a lieu
      if( aladinMenu!=null ) {
   ...
   /** Test l'�galit� du nom du serveur en prenant en compte les \n qui auraient
    * pu �tre remplac� par ' ' */
   ...
   /** Positionne si possible le param�tre principal dans le formulaire
    * (voir ArchiveServer) */
 ...
 /** Retourne le premier mot du nom */
   ...
   /** Retourne un label associ� au plan g�n�r� par ce serveur.
    * Si s est vide ou null, ou ne contient qu'un identificateur technique "as id" retourne 
    * un label par d�faut. Dans le dernier cas le label par d�faut sera ins�r� en pr�fixe.
    * Dans tous les autres cas retourne le label propos�. */
//   protected String getDefaultLabelIfRequired(String s) { return getDefaultLabelIfRequired(s,aladinLabel); }
 ...
 /** Retrouve le frame.
   ...
   */
   ...
   /**
    * Retourne la ligne de description d'un filtre pr�d�fini
    * @param filter le filtre selon la syntaxe d�crite dans filters[]
   ...
   /**
    * Retourne le nom d'un filtre pr�d�fini
    * @param filter le filtre selon la syntaxe d�crite dans filters[]
...
//      System.out.println("==> ["+s+"]");
   ...
   /**
    * Retourne le texte d'un filtre pr�d�fini sans les commentaires
    * @param filter le filtre selon la syntaxe d�crite dans filters[]
   ...
   /**
    * Retourne l'indice du filtre pr�d�fini
    * @param filters le tableau des filtres pr�d�finis
    * @param name le nom ou la description du filtre dont on cherche l'indice
    * @return l'indice du filtre, ou -1 si non trouv�
    */
   ...
   /**
    * Retourne le script d'un filter pr�d�fini
    * @param filter le filtre selon la syntaxe d�crite dans filters[]
    * @return le script du filtre ou null si probl�me
    */
   ...
   /**
    * Creation d'un choice en fonction des filtres pr�d�finis du serveur
    * (cf filters[])
    */
   ...
   /** Peuple la Combobox des filtres d�di�s */
   ...
   /** Surcharge �ventuelle des filtres pr�d�finis - par exemple pour Simbad ou NED.
    * Ceci ne concerne pas les serveurs automatiquement cr��s par le GLU, car
    * ils ont d�j� leur d�finition de filtres pr�d�finis */
   ...
   /**
    * Retourne le num�ro du filtre s�lectionn� dans filtersChoice, -1 si aucun ou probl�me
    */
...
//      if( nFilter==-1 ) return nFilter;
   ...
   /** Positionnement du mode du voyant d'�tat en fonction du dernier plan
    ...
    * @param y positionnement en ordonn�e
    * @return nouvelle ordonn�e
    */
...
//   }
   ...
   /**
	 * Method parses adql query from user using Gr�gory
